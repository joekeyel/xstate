...
const callBack = {
  ...
  states: {
    ...
    evaluatingCode: {
      on: {
        '': [
          { target: '#idaas.loggedOut.loggingIn.gettingAccessToken', cond: 'codeExist' },
          {
  ...
  checklocalStorage:{
   on :{'':
    [
     ...
     {target:'#idaas.loggedIn',cond:'tokenExistIntStorage'}
    ...
    checkContext:{
      on :{'':
       [
        {target:'#idaas.loggedOut.browsing',cond:'invalidtoken'},
...
const loggedIn = {
  ...
  states: {
            ...
            on:{
                ...
                LOGOUT : {
                    ...
                    'clearLocalStorageIdaas',
                      ...
                      assign({ idaasUserInfo: (ctx,event) => ('')  }),

                      assign({ idaas: (ctx,event) => ('')  }),
    ...
    refreshingToken : {
      ...
      invoke: {
        ...
        src: (ctx, event) => {
          return fetch(ctx.token_endpoint, {
            method: 'POST',
            body: `${ctx.idaasConfig.refresh_token_body()}&refresh_token=${ctx.idaas.refresh_token}`,
            headers: { 'Content-type': 'application/x-www-form-urlencoded' }
          }).then(response => response.json())
           .catch(error => console.error(error))
        },
    ...
    evaluatingRefreshToken: {
      on: {
        '': [
          { target: 'countTokenExpired', cond: 'validtoken' },
          {
            target: '#idaas.loggedOut.error',
            cond: 'invalidtoken'
